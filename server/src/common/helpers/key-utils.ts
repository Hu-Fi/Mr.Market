...
export function parsePrivateKeyToUint8Array(input: string): Uint8Array {
    ...
    if (s.startsWith('[') && s.endsWith(']')) {
      ...
      if (!Array.isArray(arr)) throw new Error('Invalid JSON array for private key');
    ...
    throw new Error('Unsupported private key format. Use solana-keygen JSON array, hex, or base64.');
  }
