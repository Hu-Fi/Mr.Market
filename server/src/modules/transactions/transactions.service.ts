...
export class TransactionsService {
  ...
  async createDeposit(userId: string, amount: number, currency: string, exchange: string): Promise<Transaction> {
...
//   }
  ...
  private async updateBalance(userId: string, exchange: string, currency: string, amount: number): Promise<void> {
    let userBalance = await this.userBalanceRepository.findOne({ where: { userId, exchange, currency } });
