...
describe('MixinListener', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [MixinListener, SnapshotsService, ExchangeService, CustomConfigService],
    }).compile();
  ...
  it('should fail when assetId is invalid', async () => {
    ...
    expect(exchangeService.updateSpotOrderState).toHaveBeenCalledWith(mockEvent.orderId, STATE_TEXT_MAP['MIXIN_RELEASE_FAILED']);
  });
  ...
  it('should handle duplicate release attempt gracefully', async () => {
    ...
    await listener.handleReleaseTokenEvent(mockEvent);
    expect(exchangeService.updateSpotOrderState).not.toHaveBeenCalledWith(mockEvent.orderId, STATE_TEXT_MAP['MIXIN_RELEASED']);
  });
  ...
  it('should successfully release token and record history', async () => {
    ...
    snapshotsService.sendMixinTx.mockResolvedValue([{
      ...
      'transaction_hash': 'e4fe30aa3f222f7de1518c8446dbd030d11a506faabcfe8592f9743d15bedb16',
      'asset': 'USD',
      'amount': '13422',
      ...
      'snapshot_hash': 'fc7607d1e5bdc03e4ad64145882e8f80df7628e161d99db16cf4c16f5dcf22b9',
      ...
      'senders_hash': 'b155d77c4018bc29190117a963300f15f62644a035898fafa056f15eae1c786e',
      'senders_threshold': 2,
      'signers': ['36b88829-10c6-4d9f-a34d-e46fead6f6eb'],
      'views': ['Address amount.']
    }]);
    await listener.handleReleaseTokenEvent(mockEvent);
    expect(exchangeService.updateSpotOrderState).toHaveBeenCalledWith(mockEvent.orderId, STATE_TEXT_MAP['MIXIN_RELEASED']);
  });
