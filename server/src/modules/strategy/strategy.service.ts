...
export class StrategyService {
    ...
    async stopArbitrageStrategyForUser(userId: string, clientId: string) {
        ...
        if (strategyInstance) {
            ...
            this.logger.log(`Stopped arbitrage strategy for user ${userId}, client ${clientId}`);
    ...
    private async evaluateArbitrageOpportunityVWAP(exchangeA, exchangeB, userId: string, clientId: string, symbol: string, amountToTrade: number, minProfitability: number) {
        ...
        if (cachedOrderBookA && cachedOrderBookB && this.isDataFresh(cachedOrderBookA.timestamp) && this.isDataFresh(cachedOrderBookB.timestamp)) {
            const vwapA = this.calculateVWAPForAmount(cachedOrderBookA.data, amountToTrade);
            const vwapB = this.calculateVWAPForAmount(cachedOrderBookB.data, amountToTrade);

            if ((vwapB - vwapA) / vwapA >= minProfitability) {
                this.logger.log(`User ${userId}, Client ${clientId}: Arbitrage opportunity for ${symbol} (VWAP): Buy on ${exchangeA.name} at ${vwapA}, sell on ${exchangeB.name} at ${vwapB}`);
                // Implement trade execution logic or further analysis
            }

            if ((vwapA - vwapB) / vwapB >= minProfitability) {
                this.logger.log(`User ${userId}, Client ${clientId}: Arbitrage opportunity for ${symbol} (VWAP): Buy on ${exchangeB.name} at ${vwapB}, sell on ${exchangeA.name} at ${vwapA}`);
                // Implement trade execution logic or further analysis
            }
        } else {
    ...
    private calculateVWAPForAmount(orderBook: ccxt.OrderBook, amountToTrade: number): number {
