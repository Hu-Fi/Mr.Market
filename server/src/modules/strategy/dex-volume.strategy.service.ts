...
export class DexVolumeStrategyService {
  ...
  async execute(dto: DexVolumeStrategyDto) {
    ...
    const poolAddr = await adapter.getPool(
      provider,
      chainId,
      tokenIn,
      tokenOut,
    ...
    if (poolAddr === ethers.constants.AddressZero) {
      throw new Error(
        `No pool for ${tokenIn}/${tokenOut} fee ${feeTier} on chain ${chainId}`,
      );
    }
    ...
    const oneCycle = async () => {
      ...
      try {
        ...
        const amtInHuman = clampJitter(
          parseFloat(baseAmountIn),
          amountJitterPct,
        );
        const amountIn = ethers.utils.parseUnits(
          amtInHuman.toFixed(decIn),
          decIn,
        );

        // Quote A->B
        const { amountOut } = await adapter.quoteExactInputSingle(
          provider,
          chainId,
          {
            tokenIn,
        ...
        const impactPct =
          Math.abs((effOutPerIn - spotOutPerIn) / spotOutPerIn) * 100;
        if (impactPct > maxPriceImpactPct) {
          this.logger.warn(
            `[${strategyKey}] Impact ${impactPct.toFixed(
              4,
            )}% > cap ${maxPriceImpactPct}%. Skip.`,
          );
          return scheduleNext();
        }
        ...
        if (maxGasCostInQuote && gasTokenPriceInQuote) {
          const gas = await adapter.estimateGasExactInputSingle(
            signer,
            chainId,
            {
              tokenIn,
              ...
              fee: feeTier,
              recipient: fromAddr,
              deadline: this.nowUnix() + deadlineSec,
              amountIn,
          ...
          const price =
            feeData.maxFeePerGas ?? feeData.gasPrice ?? BigNumber.from(0);
          const costNative = gas.mul(price);
          const costInQuote =
            parseFloat(ethers.utils.formatEther(costNative)) *
            gasTokenPriceInQuote;
          if (costInQuote > maxGasCostInQuote) {
            this.logger.warn(
              `[${strategyKey}] Gas ${costInQuote.toFixed(
                6,
              )} > cap ${maxGasCostInQuote}. Skip.`,
            );
            return scheduleNext();
          }
        }
        ...
        if (tokenIn !== ethers.constants.AddressZero) {
          await ensureAllowance(
            ...
            tokenIn,
            fromAddr,
            adapter.getAddresses(chainId).router,
            amountIn,
          );
        }
        ...
        if (dryRun) {
          this.logger.log(
            `[${strategyKey}] DRY A→B: in=${amtInHuman} out≈${ethers.utils.formatUnits(
              amountOut,
              decOut,
            )} min=${ethers.utils.formatUnits(
              minOut,
              decOut,
            )} impact=${impactPct.toFixed(3)}%`,
          );
        } else {
          ...
          this.logger.log(
            `[${strategyKey}] Swap A→B ok: ${rcpt.transactionHash}`,
          );
        }

        // Optional immediate B->A (bounded by available balance)
        const balOut = await new ethers.Contract(
          tokenOut,
          ...
          if (!dryRun) {
            ...
            await this.orderRepo.save(
              this.orderRepo.create({
                ...
                side: 'sell',
                amount: parseFloat(
                  ethers.utils.formatUnits(amountBack, decOut),
                ),
            ...
            this.logger.log(
              `[${strategyKey}] Swap B→A ok: ${rcpt2.transactionHash}`,
            );
          } else {
            this.logger.log(
              `[${strategyKey}] DRY B→A: in≈${ethers.utils.formatUnits(
                amountBack,
                decOut,
              )} out(min)≈${ethers.utils.formatUnits(minBack, decIn)}`,
            );
          }
        }
    ...
    const scheduleNext = () => {
      const delay = Math.max(
        1000,
        Math.floor(clampJitter(baseIntervalSec, intervalJitterPct) * 1000),
      );
      const t = setTimeout(oneCycle, delay);
      this.loops.set(strategyKey, t);
      this.logger.log(
        `[${strategyKey}] Next hop in ~${Math.round(delay / 1000)}s`,
      );
    };

    const scheduleRetry = () => {
      const delay = Math.floor(
        clampJitter(baseIntervalSec * 1.5, intervalJitterPct) * 1000,
      );
      const t = setTimeout(oneCycle, delay);
      this.loops.set(strategyKey, t);
      this.logger.log(
        `[${strategyKey}] Retry in ~${Math.round(delay / 1000)}s`,
      );
    };

    this.logger.log(
      `[${strategyKey}] DEX volume started on ${dexId}@${chainId}`,
    );
    oneCycle();
  }
