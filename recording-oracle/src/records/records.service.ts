...
export class RecordService {
  ...
  private getExchangeInstance(exchangeId: string, apiKey: string, secret: string): ccxt.Exchange {
  ...
  private async fetchTrades(exchange: ccxt.Exchange, symbol: string, since: number): Promise<ccxt.Trade[]> {
    return exchange.fetchMyTrades(symbol, since);
  }

  private async fetchOpenOrders(exchange: ccxt.Exchange, symbol: string): Promise<ccxt.Order[]> {
    return exchange.fetchOpenOrders(symbol);
  }

  private async fetchOrderBook(exchange: ccxt.Exchange, symbol: string): Promise<ccxt.OrderBook> {
  ...
  private async processOpenOrders(exchange: ccxt.Exchange, symbol: string): Promise<{ openOrderVolume: number, averageDuration: number, spread: number }> {
  ...
  async calculateLiquidityScore(apiKey: string, secret: string, exchangeId: string, symbol: string, since: number): Promise<number> {
    ...
    const { openOrderVolume, averageDuration, spread } = await this.processOpenOrders(exchange, symbol);
