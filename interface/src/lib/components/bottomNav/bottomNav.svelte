...
<div class={clsx("btm-nav btm-nav-xs h-[3.25rem] visible bg-base-100", $theme==="dark"?"shadow-zinc-800":"shadow-zinc-50")}>
  {#each routes as route, i}
    <button class={clsx("")} on:click={async ()=> {await goto(route.route);}}>
      ...
      <span class={clsx("text-xs !text-[10px] absolute top-[30px]",active === i ? 'opacity-100 text-primary' : 'opacity-40')}>{route.title}</span>
    </button>
  {/each}
</div>
