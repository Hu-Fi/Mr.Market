<script lang="ts">
  ...
  import AppIcon from "../home/applications/appIcon.svelte";
  
  $: active = $page.url.pathname.includes('/home') ? 0 : 
    $page.url.pathname.includes('/market_making') ? 3 :
  ...
  $: routes = [
    ...
    {icon: 'market_making', route: '/market_making', title: $_('mm_short')},
...
<div class={clsx(
  ...
  {#each routes as route, i}
    <button
      type="button"
      class={clsx(
        "dock-item text-xs transition-colors",
        active === i ? "text-base-content" : "text-base-content/60",
        route.icon === 'trade' && "dock-item-trade"
      )}
      aria-current={active === i ? "page" : undefined}
      on:click={()=>routing(route.route)}
      data-testid={`bottom-nav-${route.icon}`}
    >
      <div class={clsx(
        "flex items-center justify-center",
        route.icon === 'trade' ? "trade-icon-wrapper" : "h-5 w-5"
      )}>
        {#if route.icon === 'trade'}
          <!-- Trade -->
          <span class={clsx(
            "trade-icon flex items-center justify-center rounded-full bg-slate-800 text-base-100",
            active === i ? "trade-icon-active" : "trade-icon-inactive"
          )}>
        ...
        {:else if route.icon === 'market_making'}
          <!-- Market making -->
          <AppIcon icon="market_making" />
