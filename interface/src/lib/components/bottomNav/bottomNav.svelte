...
<div class={clsx("btm-nav btm-nav-xs h-[3.25rem] visible bg-base-100 shadow-[0_-1px_1px_black]", $theme==="dark"?"shadow-zinc-800":"shadow-zinc-100")}>
  {#each routes as route, i}
    <button class={clsx("flex flex-col justify-start")} on:click={async ()=> {await goto(route.route);}}>
      <div class={clsx(active === i ? 'opacity-100 text-base-content' : i===2 ? '' : 'opacity-40', "mt-2")}>
        {#if route.icon === 'trade'}
          <!-- Trade -->
          <button class={clsx(active === i ? "trade-icon-active" : "trade-icon-inactive" ," relative bottom-7 trade-icon p-2.5 flex justify-center items-center bg-base-content rounded-full")} on:click={()=>{$page.url.pathname==='/trade' ? bottomTradeDialog.set(true): goto('trade')}}>
