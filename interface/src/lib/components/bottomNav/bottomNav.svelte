  ...
  {#each routes as route, i}
    <button class={clsx("flex flex-col justify-start")} on:click={()=>routing(route.route)} data-testid={`bottom-nav-${route.icon}`}>
      <div class={clsx(active === i ? 'opacity-100 text-base-content' : i===2 ? '' : 'opacity-40', "mt-2")}>
        {#if route.icon === 'trade'}
          <!-- Trade -->
          <button class={clsx(active === i ? "trade-icon-active" : "trade-icon-inactive" ," relative bottom-7 trade-icon p-2.5 flex justify-center items-center bg-base-content rounded-full")}>
