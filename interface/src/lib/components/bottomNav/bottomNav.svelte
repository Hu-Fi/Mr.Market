...
<div class={clsx("btm-nav btm-nav-xs h-[3.25rem] visible bg-base-100", $theme==="dark"?"shadow-zinc-800":"shadow-zinc-50")}>
  {#each routes as route, i}
    <button class={clsx("flex flex-col justify-start")} on:click={async ()=> {await goto(route.route);}}>
      <div class={clsx(active === i ? 'opacity-100 text-primary' : i===2 ? '' : 'opacity-40', "mt-2")}>
        {#if route.icon === 'trade'}
          <!-- Trade -->
          <button class={clsx(active === i ? "trade-icon-active" : "trade-icon-inactive" ," relative bottom-7 trade-icon p-2.5 flex justify-center items-center bg-gradient-to-r from-sky-400 to-blue-500 rounded-full")} on:click={()=>{$page.url.pathname==='/trade' ? bottomTradeDialog.set(true): goto('trade')}}>
