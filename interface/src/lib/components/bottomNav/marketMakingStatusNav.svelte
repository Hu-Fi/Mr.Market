<script lang="ts">
  ...
  import { currentMarketMakingStatus } from "$lib/stores/grow";
...
<div class="btm-nav btm-nav-md shadow-2xl bg-base-100 visible md:invisible px-4">
  <div class="flex flex-row justify-start space-x-2">  
    <!-- Stop/Delete Button -->
    <button class="btn bg-base-100 hover:bg-base-100 border-base-300 no-animation flex border items-center justify-center rounded-full space-x-0 px-6" on:click={()=>{goto(`/trade`)}}>
      {#if statusStop}
    ...
    <button 
      ...
      on:click={()=>{console.log('open Add/Remove dialog')}}
    >
