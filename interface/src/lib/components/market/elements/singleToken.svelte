...
<button
  class={clsx(
    "flex w-full items-center justfy-between p-4 border-b-[0.1px] first:border-t-[0.1px]",
    $darkTheme ? "border-slate-800" : "border-slate-50",
  )}
  on:click={() => {
    goto("/market/token/" + token.id);
    currentCoin.set(token);
  }}
>
  <div class="flex flex-1 items-center space-x-3 text-left">
    ...
    <div class="avatar w-9 h-9 p-0.5">
      <div class="rounded-full">
        <img
          src={findCoinIconBySymbol(token.symbol.toUpperCase()) ||
            token.image ||
            emptyToken}
          alt=""
          class="w-8 h-8"
    ...
    <div class="flex flex-col space-y-0.5">
      ...
      <div class="flex space-x-1.5 items-center">
        {#if token.market_cap_rank}
          <div
            class={clsx(
              "flex rounded-sm badge badge-xs px-0.5",
  ...
  <div class="flex space-x-2">
    ...
    <div class="w-[4.5rem] flex items-center justify-end">
      <div
        class={clsx(
          "min-w-14 w-[4rem] h-7 flex justify-center items-center rounded-md px-2",
