<script lang="ts">
  ...
  import { BN, formatDecimals, formatUSMoney } from "$lib/helpers/utils";
  import { findExchangeIconByIdentifier } from "$lib/helpers/helpers";

  export let pair: PairsData;
  $: pairPriceValid = BN(pair.price).isGreaterThan(0)
  $: pairChangeValid = BN(pair.change).isGreaterThan(0)
</script>
  ...
  <div class="flex items-center space-x-2">
    <div>
      <span class="text-xs font-semibold">
        {pairPriceValid ? formatUSMoney(pair.price) : '---'}
      </span>
    </div>
