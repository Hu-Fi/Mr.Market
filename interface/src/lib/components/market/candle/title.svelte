...
<div class="flex items-center space-x-4 pl-4">
  <button on:click={()=>{history.back(); if (!history.length) goto('/trade') }}>
    <!-- Chevron left Icon -->
...
<div class="flex items-center space-x-4">
  ...
  <button class="" on:click={()=>{mixinShare($page.url.pathname, $_('coin_candle', {values:{symbol:$currentCoin.symbol.toUpperCase()}}), AppName, findExchangeIconByIdentifier($currentCoin.exchange))}}>
