<script lang="ts">
  import OrderBookDecimal from "$lib/components/dialogs/orderBookDecimal.svelte";
  import { orderBookDecimal, orderBookDecimalDialog, orderBookMode } from "$lib/stores/trade";
...
<div class="flex space-x-3">
  <button class="btn btn-xs btn-outline border-base-300 hover:bg-base-200 hover:border-base-300 hover:text-base-content w-20 rounded-md text-start justify-between no-animation" on:click={()=>{orderBookDecimalDialog.set(true)}}>
    <span class="">
      {$orderBookDecimal}
    </span>
    {#if !$orderBookDecimalDialog}
      <!-- Caret down Icon -->
