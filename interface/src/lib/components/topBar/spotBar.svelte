<script lang="ts">
  ...
  import { pair, pairSelectorDialog, pairBaseSymbol, pairTargetSymbol } from "$lib/stores/spot";
...
<div class="flex md:px-0 items-center justify-between py-[4pt] my-[4pt] !h-[36px] !min-h-[36px] mr-[6px]">
  ...
  <button class="flex items-center pl-4" on:click={()=>pairSelectorDialog.set(true)} data-testid="spot_pair_selector">
    <img src={findExchangeIconByIdentifier($pair.exchange_id)} alt="icon" loading="lazy" class="w-5 h-5 mr-1" />
    <span class="font-black text-lg" title="pair-name"> {$pairBaseSymbol+"/"+$pairTargetSymbol} </span>
  ...
  <div class="flex items-center space-x-4">
    <!-- CandleStick -->
    <button on:click={()=>{CandlePair.set($pair); goto(`/market/candle/${$pair.exchange_id}/${$pairBaseSymbol+"-"+$pairTargetSymbol}`)}}>
