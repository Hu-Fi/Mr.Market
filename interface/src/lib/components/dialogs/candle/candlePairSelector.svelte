<script lang="ts">
  ...
  import { switchCandleStickPair } from "$lib/helpers/mrm/socket";
  ...
  import {
    CandlePairSelectorDialog as sd,
    CandlePairSearch,
    CandlePairExchangeFilter,
    CandlePairSelectorLoaded,
    CandleChart,
  } from "$lib/stores/market";

  let items = [
    { name: "all" },
    ...SUPPORTED_EXCHANGES.filter((e) => {
      return e != "bitfinex";
  ...
  $: filteredPairs = pairs
    ? pairs
        .filter((item) => {
          return $CandlePairExchangeFilter.toUpperCase() === "ALL"
            ? item
            : item.exchange
                .toUpperCase()
                .match($CandlePairExchangeFilter.toUpperCase());
        })
        .filter((item) => {
          return (
...
<dialog
  id="candle_select_pair_modal"
  class="modal modal-bottom sm:modal-middle"
  class:modal-open={$sd}
>
  <div
    class="modal-box h-[90vh] pt-0 px-0"
    data-testid="candlestick_pair_selector_modal_box"
  >
      ...
      <div class="absolute left-[calc(50%-16px)] top-2">
        <div class="w-8 h-1 bg-slate-800/40 rounded-full">
          <form method="dialog" class="modal-backdrop">
            <button
              on:click={() => sd.set(false)}
              data-testid="candlestick_pair_selector_close">c</button
            >
          </form>
        </div>
      </div>
      ...
      <div class="tabs border-b w-full mt-3 px-3 overflow-x-auto no-scrollbar">
        {#each items as item}
          <button
            class={clsx(
              "tab",
              $CandlePairExchangeFilter === item.name &&
                "border-b border-base-content",
            )}
            on:click={() => {
              CandlePairExchangeFilter.set(item.name);
            }}
          >
            <span
              class={clsx(
