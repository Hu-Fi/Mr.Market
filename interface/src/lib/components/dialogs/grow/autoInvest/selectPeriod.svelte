<script lang="ts">
  ...
  import { createAIPeriodDialog } from "$lib/stores/grow";
</script>
...
>
  <div class="modal-box space-y-3 pt-0 pl-6">
      ...
      <div class="absolute left-[calc(50%-16px)] top-2">
        <div class="w-8 h-1 bg-base-content/40 rounded-full">
          <form method="dialog" class="modal-backdrop">
            <button on:click={() => createAIPeriodDialog.set(false)}>c</button
            >
          </form>
        </div>
      </div>
    ...
    <div class="flex flex-col">
      ...
      <button class="flex space-x-6 my-6 text-start items-center" on:click={()=>{ createAIPeriodDialog.set(false)}}>
