<script lang="ts">
  ...
  import { InputAsset, InputBalanceAccount, InputBalanceDialog } from "$lib/stores/swap";

  const accounts = [
    { name: $_("mixin_wallet"), key: 'mixin', balance: 21.2134 },
    { name: $_("trading_account"), key: 'trading', balance: 324.1234 },
  ];
  onMount(()=>{ InputBalanceAccount.set(accounts[0]) })
  let active = 'mixin'
</script>
  ...
  <div class="modal-box space-y-3 pt-0">
    ...
    <div class="flex flex-col !mt-6">
      {#each accounts as acc, i}
        <button class="flex items-center justify-between" on:click={()=>{active = acc.key; InputBalanceAccount.set(acc); InputBalanceDialog.set(false)}} data-testid={`input-account-${i}`}>
