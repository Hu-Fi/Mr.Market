<script lang="ts">
  ...
  import { InputAsset, InputAssetDialog, InputAssetSearch } from "$lib/stores/swap";
  ...
  <div class="modal-box h-[90vh] pt-0">
    <div class="sticky top-0 bg-opacity-100 bg-base-100 z-10 py-4 flex flex-col space-y-4">
      ...
      <div class="join w-full">
        ...
        <input bind:value={$InputAssetSearch} class="input input-md h-[2.5rem] w-full pl-2 focus:outline-none focus:border-base-200 rounded-full bg-base-200 join-item" placeholder={$_('search')} />
      </div>
    </div>
    ...
    <div class="flex flex-col">
      ...
      {:else}
        {#each filteredAssets as asset, i}
          <SingleSwapAsset {asset} fn={()=>{InputAsset.set(asset); InputAssetDialog.set(false)}} testid={i}/>
        {/each}
      {/if}
    </div>
  </div>
