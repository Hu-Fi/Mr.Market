<script lang="ts">
  ...
  import { orderBookModeDialog, orderBookMode } from "$lib/stores/spot";
...
<div class="flex space-x-3">
  <button class="btn btn-xs grow btn-outline border-base-300 hover:bg-base-200 hover:border-base-300 hover:text-base-content w-20 rounded-md text-start justify-between no-animation" on:click={()=>{orderBookModeDialog.set(true)}}>
    <span>
