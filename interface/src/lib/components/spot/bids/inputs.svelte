<script lang="ts">
  ...
  import { pair, price, amount, total, buy, orderType, current } from "$lib/stores/spot";
  ...
  const auth = async () => {
    ...
    authorize( { clientId: BOT_ID, scope: OAUTH_SCOPE, pkce: true }, { onShowUrl: (url: string) => { window.open(url) }, onError: (error: Error) => { console.error(error); return; }, onSuccess: async (token: string) => { await AfterMixinOauth(token); }, }, );
...
<div>
  ...
  <div class="flex items-center justify-between mb-1 py-1">
    {#if $mixinConnected}
      ...
      <button class="" on:click={()=>{setBalance()}}><span class="text-xs opacity-90">{balance} {$pair.symbol.split('/')[1]}</span></button>
    {:else}
