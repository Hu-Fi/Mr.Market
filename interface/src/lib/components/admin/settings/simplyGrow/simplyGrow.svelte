<script lang="ts">
    ...
    async function AddSimplyGrowToken(growToken: SimplyGrowTokenDto) {
      ...
      if (!token) return;
      addSimplyGrowToken(growToken, token).then(() => {
        setTimeout(() => {
        ...
        <tbody>
          ...
          {#each simplyGrowTokens as token}
            <tr>
              ...
              <td>
                <div class="tooltip" data-tip={isUpdating === token.asset_id ? $_("updating") : token.enable ? $_("click_to_disable") : $_("click_to_enable")}>
                  <button on:click={async () => {
                    const newEnable = !token.enable;
                    await UpdateSimplyGrowToken(token.asset_id, newEnable);
                  }}>
                    <span class={clsx(isUpdating === token.asset_id && "loading loading-spinner loading-xs disabled")}>
                      {token.enable ? '✅' : '❌'}
                    </span>
                  </button>
                </div>
              ...
              <td>
                <button class="btn btn-ghost rounded-2xl btn-xs px-2" on:click={async () => { 
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class={clsx("size-4", isDeleting === token.asset_id && 'loading loading-spinner')}>
        ...
        <div class="menu dropdown-content bg-base-100 rounded-box z-[1] p-2 shadow">
          <div class="p-2 flex flex-col gap-4"> 
            ...
            <div class="flex flex-col gap-2">
              ...
              <input type="text" class="input input-bordered focus:outline-none" bind:value={AddNewAssetId} />
            </div>
            <div class="flex flex-col gap-2">
              ...
              {#if !AddNewIconUrl && validate(AddNewAssetId)}
                <button class="btn btn-bordered btn-md no-animation" on:click={async () => {
                  ...
                  <span class={clsx(isIconFetching && "loading loading-spinner loading-sm")}>
                    {$_("fetch")}
                  </span>
                </button>
              {:else}
                <input type="text" class="input input-bordered focus:outline-none" bind:value={AddNewIconUrl} />
              {/if}
            </div>
            <div class="flex flex-col gap-2">
              <span class="label"> {"apy"} </span>
              <input type="text" class="input input-bordered focus:outline-none" bind:value={AddNewApy} />
            </div>
