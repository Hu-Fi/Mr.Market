<script lang="ts">
  ...
  import { createSimplyGrowAmount, createSimplyGrowAsset } from "$lib/stores/grow";

  onDestroy(() => {
    createSimplyGrowAmount.set({ key: '', value: '' });
  })
</script>
...
<div class="flex flex-col space-y-2 mx-4">
  ...
  <div class="flex items-center justify-between space-x-2 mx-2">
    <div class="flex space-x-2 items-center">
      <AssetIcon assetIcon={findCoinIconBySymbol($createSimplyGrowAsset.symbol) || $createSimplyGrowAsset.icon_url} clazz="w-8 h-8" claxx="w-2 h-2" />
