...
<div class="mx-4 mt-2">
  <div class={clsx("w-full grid gap-2 rounded-2xl", `grid-cols-${items.length}`)}>
    {#each items as item, i}
      ...
      >
        <div class={clsx("flex items-center justify-center rounded-full p-2.5 border border-slate-200", i === 0 && 'text-blue-600 border-blue-600', i === 1 && 'text-green-600 border-green-600')}>
          {#if i === 0}
            <!-- Arbitrage -->
        ...
        <span
          class={clsx(
            ...
            i === 0 && 'text-blue-600', i === 1 && 'text-green-600',
          )}>{item.name}</span
        >
      </button>
    {/each}
