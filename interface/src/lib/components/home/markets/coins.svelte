<script lang="ts">
  ...
  import { marketData, CoinMarketCategory } from "$lib/stores/market";
  ...
  const handleSuccess = x => { resolved = true, failed = false, defaults=x, marketData.set(x) };
  ...
  $page.data.market.then(handleSuccess).catch(handleFailure)
</script>
