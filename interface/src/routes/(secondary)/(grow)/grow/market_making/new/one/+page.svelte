      ...
      <div class="flex space-x-2 items-center overflow-x-auto no-scrollbar">
        ...
        {:then growInfo}
          {#each growInfo.exchanges as exchange}
            ...
            >
              <div
                class="flex items-center justify-center mx-5 space-x-1 py-1.5"
              >
                <img
                  src={findExchangeIconByIdentifier(exchange.exchange_id)}
                  alt=""
      ...
      {:then growInfo}
        <div class="grid grid-cols-2 gap-4 w-full overflow-y-auto">
          {#each growInfo.market_making.pairs as item, i}
            <!-- Filter by exchange -->
            {#if showAllExchanges || item.exchange_id
                .toUpperCase()
                .match($createMMSelectPairEasyFilter.toUpperCase())}
              <!-- Filter by search -->
              {#if item.enable && ($createMMSelectPairEasySearch.toUpperCase() === "" || item.symbol
                    .toUpperCase()
